...
class Qwen3MultiHeadAttention:
    def __init__(
        ...
        self.wv = wv
        self.wo = wo
        # Week 3 always runs the fast Week 2 kernels around the projections.
    ...
    def __call__(
        ...
        if self.use_paged_attention:
            # Pages are the source of truth: append into the pool and hand the
            # kernel block_table/context_lens instead of dense K/V.
...
class Qwen3ModelWeek3:
    def __init__(
        ...
        # Each layer owns physical storage shared by all request caches for
        # that layer, so page ids are layer-local like in the kernel.
        self.page_pools = [
    ...
    def create_kv_cache(self) -> list[TinyKvCache]:
        # One request gets one cache handle per layer; handles share the layer
        # pool's storage but keep independent logical metadata.
