...
// Week 2, Day 4. One threadgroup of 256 threads (8 SIMD groups) per row:
// each group reduces a strided slice with simd_sum, lane 0 publishes eight
// partial sums to threadgroup memory, group 0 finishes the reduction, then
// every lane normalizes and scales its strided elements.
void Week2RMSNorm::eval_gpu(const std::vector<mx::array> &inputs, std::vector<mx::array> &outputs) {
    ...
    auto kernel = d.get_kernel(std::string("week2_rms_norm_") + dtype_suffix(out), d.get_library("tiny_llm_ext"));
...
// One thread covers a pair index inside a block of four heads: the rotation
// angle is computed once and reused across those heads. Tail head-dim
// elements beyond `dims` are copied through.
void Week2RoPE::eval_gpu(const std::vector<mx::array> &inputs, std::vector<mx::array> &outputs) {
    ...
    out.set_data(mx::allocator::malloc(out.nbytes()));
    auto &d = mx::metal::device(stream().device);
    auto kernel = d.get_kernel(std::string("week2_rope_") + dtype_suffix(out), d.get_library("tiny_llm_ext"));
    auto &encoder = mx::metal::get_command_encoder(stream());
    encoder.set_compute_pipeline_state(kernel);
    encoder.set_input_array(x, 0);
...
// One thread per element: SiLU on the gate branch with one exponential, then
// the element-wise multiply with up.
void Week2SwiGLU::eval_gpu(const std::vector<mx::array> &inputs, std::vector<mx::array> &outputs) {
    ...
    out.set_data(mx::allocator::malloc(out.nbytes()));
    auto &d = mx::metal::device(stream().device);
    auto kernel = d.get_kernel(std::string("week2_swiglu_") + dtype_suffix(out), d.get_library("tiny_llm_ext"));
    auto &encoder = mx::metal::get_command_encoder(stream());
    encoder.set_compute_pipeline_state(kernel);
...
// Week 2, Day 5. One threadgroup per query row: 32 SIMD groups sweep the
// context cooperatively with an online softmax, then the partial results are
// merged through threadgroup memory and the normalized output written.
void Week2DecodeAttention::eval_gpu(const std::vector<mx::array> &inputs, std::vector<mx::array> &outputs) {
    ...
    auto &d = mx::metal::device(stream().device);
    auto kernel =
        d.get_kernel(std::string("week2_decode_attention_") + dtype_suffix(out), d.get_library("tiny_llm_ext"));
    auto &encoder = mx::metal::get_command_encoder(stream());
    encoder.set_compute_pipeline_state(kernel);
