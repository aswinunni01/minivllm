...
// Week 3, Day 3: slice-sized device write into one physical page. One thread
// per appended element; the grid covers H * new_tokens * D values only.
template <typename T>
...
// Week 3, Day 4: direct paged decode attention.
//
// One threadgroup (32 SIMD groups) per flattened query row [B*Hq, L]. Each
// SIMD group sweeps a strided slice of the VISIBLE context - walking the
// block table page by page instead of gathering dense K/V first - keeping an
// online-softmax state in registers. Scores use exp2 with a log2(e)-scaled
// query to reuse the fast hardware path. Partials merge through a compact
// 32x32 shared tile so every SIMD group reduces one output lane.
...
[[kernel]] void paged_attention_decode(
    ...
    // GQA: consecutive query heads share one KV head.
...
#pragma clang loop unroll(full)
    ...
    // Causal rows see keys up to context - L + query_position.
...
#pragma clang loop unroll(full)
    ...
    // Merge per-SIMD-group states through shared memory: rescale every group
    // to the global max, then transpose the partial output vectors so each
    // SIMD group reduces one output lane with simd_sum.
...
// Correctness-first FP32 prefill over pages: one threadgroup covers 16 query
// rows and a 32-key tile; probabilities stage through shared memory.
[[kernel]] void paged_attention_scalar_f32(
    ...
    if (is_causal) {
        // Tiles beyond the last query row's visibility window are skipped.
    ...
    for (int tile_idx = 0; tile_idx < kv_tile_limit; tile_idx++) {
        ...
        if (single_page_tile) {
            ...
            // A tile can straddle pages when page_size < BK or misaligned.
...
// Week 3, Day 5: FlashAttention-style long-query BF16 prefill over pages.
//
// One threadgroup covers a 64-row query block for one head: Q stages once
// into shared memory, then 32-key K/V tiles stream through it. Eight SIMD
// groups hold 8x8 matrix fragments of an online-softmax state; scores and
// probabilities stay in registers between the QK^T and PV matrix products.
// exp2 with a log2(e)-scaled query replaces base-e exponentials. Pages are
// read through the block table; a tile either sits inside one physical page
// (fast path) or gathers per-key pages when page_size does not divide 32.
[[kernel, max_total_threads_per_threadgroup(256)]] void paged_attention_mma_bf16_d128(
    ...
    int kv_tile_limit = total_kv_tiles;
    if (is_causal) {
        // The whole block can skip tiles beyond its last visible key.
        const int last_query = min((query_block + 1) * BQ, L) - 1;
        const int last_visible_key = last_query + (context - L);
        kv_tile_limit = clamp((last_visible_key + 1 + BK - 1) / BK, 0, total_kv_tiles);
    }
    ...
    for (int tile_idx = 0; tile_idx < kv_tile_limit; tile_idx++) {
        ...
        // Scores: one 8x8 fragment per 8-key sub-tile, per SIMD group.
        ...
        // Mask invalid keys, rescale the running state to the tile max, and
        ...
        // Probabilities return to BF16 for the second matrix product.
        ...
        if (single_page_tile && physical_pages[0] >= 0) {
            ...
            for (int idx = thread_idx; idx < BK * HEAD_DIM; idx += THREADS) {
                const int value_key = idx / HEAD_DIM;
                const int dim = idx - value_key * HEAD_DIM;
                const int global_key = tile_start + value_key;
                const int logical_page = global_key / page_size;
                const int slot = global_key - logical_page * page_size;
                ...
                const int page_offset =
                    ((page_id * num_kv_heads + kv_head) * page_size + slot) *
                    HEAD_DIM;
                kv_tile[value_key * LDV + dim] = page_id >= 0
        ...
        // Output += P @ V, accumulated as 8x8 fragments in FP32.
        for (int output_fragment = 0; output_fragment < OUTPUT_FRAGMENTS; output_fragment++) {
            for (int key_fragment = 0; key_fragment < SCORE_FRAGMENTS; key_fragment++) {
