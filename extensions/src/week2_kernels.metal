...
// RMSNorm, fused: one 256-thread threadgroup per row. Strided square-sum with
// simd_sum per SIMD group, a threadgroup-level second reduction through
// shared memory, then every lane normalizes and scales its strided elements.
// Accumulation stays in float; one cast at the store.
template <typename T>
...
// RoPE on the model-native B,L,H,D layout. One thread handles a pair index
// inside a block of four heads: the angle is computed once and rotated
// across all four heads. Elements beyond `dims` in each head are copied.
// Non-f32 paths use fast::exp2/cos/sin; exp2(base^-pair/half) replaces
// pow(base, -pair/half).
template <typename T>
[[kernel]] void week2_rope(
    ...
    if (item >= half_dim) {
        // Tail elements outside the rotated range pass through untouched.
    ...
    for (int h = first_head; h < last_head; ++h) {
        ...
        const int real_idx =
            traditional ? head_base + pair * 2 : head_base + pair;
        const int imag_idx =
            traditional ? real_idx + 1 : real_idx + half_dim;
...
// SwiGLU, fused: silu(gate) * up with a single exponential per element.
template <typename T>
[[kernel]] void week2_swiglu(
    ...
    out[index] =
        static_cast<T>((g / (1.0f + exp(-g))) * static_cast<float>(up[index]));
}

// Decode attention with an online softmax. One threadgroup of 32 SIMD groups
// per flattened query row [B*Hq, L]. Each SIMD group sweeps a strided slice
// of the context keeping running max/sum/value accumulators in registers;
// partials merge through threadgroup memory and the normalized output is
// written once. dim is head_dim; values_per_lane distributes it over lanes.
template <typename T>
[[kernel]] void week2_decode_attention(
    ...
    // Pre-scale the query by the softmax scale so scores need no extra pass.
    ...
    for (int position = simdgroup; position < context; position += simdgroups_per_query) {
        // Causal guard: this query position may see keys up to
        // context - length + query_position.
        ...
        for (int item = 0; item < values_per_lane; ++item) {
            ...
            if (d < dim && item < max_values_per_lane) {
                accumulator[item] = accumulator[item] * old_factor +
                                    score_factor *
                                        static_cast<float>(v[(kv_row * context + position) * dim + d]);
            }
        }
        max_score = new_max;
    }

    // Merge the per-SIMD-group online-softmax states through shared memory:
    // rescale every group's accumulator and sum to the global max.
