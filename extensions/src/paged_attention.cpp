...
mx::array paged_cache_update(const mx::array &pages, const mx::array &values, int page_id, int start,
    ...
    if ((pages.dtype() != mx::float32 && pages.dtype() != mx::bfloat16) || values.dtype() != pages.dtype()) {
        throw std::runtime_error(
            "paged_cache_update: pages and values must have the same float32 or bfloat16 dtype");
    }
...
void PagedAttention::eval_gpu(const std::vector<mx::array> &inputs, std::vector<mx::array> &outputs) {
    ...
    if (L <= 8) {
        // Decode-shaped queries: one threadgroup per query row, online softmax
        // across pages with SIMD-group cooperation.
        const char *suffix =
    ...
    if (q.dtype() == mx::bfloat16) {
        // Long-query BF16 prefill: FlashAttention-style MMA tile schedule
        // (Week 3, Day 5).
        if (D != 128) {
    ...
    // Correctness-first FP32 prefill schedule.
