...
def simple_generate(
    ...
    def _step(model, y):
        y = mx.array(y)
        model_input = y.reshape(1, *y.shape)

        output_logits = model(model_input)
        logits = output_logits[:, -1, :]
        log_probs = logits -  mx.logsumexp(logits, axis=-1, keepdims=True)
        if(sampler is not None):
            token = sampler(log_probs)
        else :
            token = mx.argmax(log_probs, axis=-1)
        mx.eval(token)
...
def simple_generate_with_kv_cache(
    ...
    def _step(model, y, offset, kv_cache):
        y = mx.array(y)
        model_input = y.reshape(1, *y.shape)
        output_logits = model(model_input, offset, kv_cache)
        logits = output_logits[:, -1, :]
        log_probs = logits - mx.logsumexp(logits, axis=-1, keepdims=True)
        token = mx.argmax(log_probs, axis=-1)
    ...
    tokens = tokenizer.encode(prompt)
    offset = 0
    token_id = _step(model, tokens, offset, kv_cache)
    offset = len(tokens)
    ...
    while True : 
        ...
        detokenizer.add_token(token_id)
        print(detokenizer.last_segment, end="", flush=True)
        token_id = _step(model, [token_id], offset, kv_cache)
        offset = offset + 1
