...
class TinyKvPagedPool:
    ...
    def key_pages(self) -> mx.array | None:
        if self._key_pages is None:
            return None
        # Callers only see allocated pages, never spare physical capacity.
    ...
    def allocate_page(self) -> int:
        # Reuse freed ids first; otherwise append a fresh page id.
    ...
    def _ensure_page_storage(self, key: mx.array, value: mx.array) -> None:
        ...
        # Geometric growth: starting at four pages and doubling keeps total
        # copy work under 2x the final capacity instead of going quadratic.
    ...
    def write_page_slice(
        ...
        # Slice-sized device write through the extension primitive: the output
        # aliases the page buffer and the kernel only touches the appended
        # slice. Page storage is request state, so this mutation boundary is
        # explicit; attention depends on the returned array.
        self._key_pages = tiny_llm_ext.paged_cache_update(
            self._key_pages,
            mx.contiguous(key),
            page_id,
            start,
        )
        self._value_pages = tiny_llm_ext.paged_cache_update(
            self._value_pages,
            mx.contiguous(value),
            page_id,
            start,
    ...
    def free_page(self, page_id: int) -> None:
        ...
        # Keep the page id stable; stale K/V bytes are harmless because
        # block_table/page_lens decide which slots are live.
...
class TinyKvPagedCache(TinyKvCache):
    ...
    Each request gets one cache handle per transformer layer. Handles for the
    same layer share a pool so pages can be recycled across requests, while
    every handle owns independent page metadata.
    """
    ...
    def _append_chunk(self, key: mx.array, value: mx.array) -> None:
        ...
        try:
            ...
            # Then allocate fresh pages for the remainder; unused tail slots
            # stay outside the logical sequence (page_lens marks the prefix).
            while start < S:
    ...
    def gather_dense(self) -> tuple[mx.array, mx.array]:
        ...
        # attention path reads pages directly via block_table/context_lens.
        key_chunks = []
        value_chunks = []
    ...
    def rewind(self, n: int):
        ...
        while len(self.page_ids) > target_num_pages:
            ...
            # allocator; stale suffix slots in the final page are ignored.
    ...
    def release(self):
        # Request completion returns every owned page to the layer allocator.
