...
def grouped_expert_linear(
    ...
    # Sort rows by expert so one gather-QMM runs each expert on a contiguous
    # block instead of launching one matmul per row; unsort afterwards.
...
def route_topk(
    ...
    # argpartition picks the top-k experts without a full sort.
...
class Moe:
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        # Each token is routed to its top-k experts; flatten token/expert
        # pairs into rows for the grouped expert matmuls.
        ...
        # Weighted sum over each token's chosen experts.
